...
const PurchaseOrderAddForm = () => {
  ...
  const onSubmit = async (data) => {
    const duplicates = findDuplicates(enteredData, 'productName');

    const lastpo = (await axios.get('https://mern-auth-liart.vercel.app/purchaseorder/po/lastnumber')).data.Ponumber
    console.log(lastpo)
    if (duplicates.length > 0) {
      ...
      if (lastpo == undefined) {
        console.log("object")
        data.PoNumber = '000001'
      }
      else {
        const po = (parseInt(lastpo) + 1)
        const poNumber = po.toString().padStart(6, '0')
        data.PoNumber = poNumber

      }
    }

    data.products = enteredData
    data.Vendor_id = poVendor
    console.log(data)
    try {
      const res = await axios.post("https://mern-auth-liart.vercel.app/purchaseorder/", {
  ...
  const handleChange = (event) => {
    const FIndFoc = Products.find((valu) => valu._id == event.target.value)
    console.log(FIndFoc)
    setData1({ foc: FIndFoc.FOC, productName: FIndFoc._id })
          ...
          <div>
            <form className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-white p-6 rounded-lg shadow-md mb-6" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div>
                ...
                <Select onChange={(vals) => setPoVendor(vals.value)} options={AllVendor.options} />
              </div>
              <div>
                  ...
                  {inputFields.map((input, index) => (
                    <div className="flex w-screen space-x-3" key={input.id}>
                      ...
                      <div>
                        <input
                          defaultValue={0}
                          onChange={(e) => enterData(e.target.value, input.id, data1)}
                      ...
                      <div>
                        <p className="px-5">
                          {enteredData.find((item) => item.id === input.id)?.value *
